...
@section('content')
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($payment->payment_status == 'Paid')
                                                <b>Approved in {{ \Illuminate\Support\Carbon::parse($payment->updated_at)->format('F d, Y h:i A') }}</b>
                                            @else
                                            <a href="{{ route('payment.approve-payment',$payment->id) }}" class="btn btn-success">Approve Payment</a>
                                            @endif
                                            <a class="btn btn-danger" onclick="return confirm('Are You Sure?')" href="{{ route('payment.delete-payment',$payment->id) }}"><i class="ti-trash"></i></a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
    ...
    <script>
        function copyToClipboard(orderId) {
            var baseUrl = "{{ route('download-khalo-digital-content') }}";
            var link = baseUrl + "?order_id=" + orderId;
            ...
            $('#copyLink'+orderId).html('Link Copied to clipboard');
        }
    </script>
    @endsection
@endsection